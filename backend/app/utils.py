...
def send_email_with_attachment(
    *,
    ...
    attachment_mimetype: str = "text/csv",
) -> None:
    assert settings.emails_enabled, "no provided configuration for email variables"
    message = emails.Message(
        subject=subject,
...
def generate_weekly_report_email(
    ...
    subject = f"{settings.PROJECT_NAME} - Weekly Report {report_date}"
    html_content = render_email_template(
        template_name="weekly_report.html",
        context={
            "project_name": settings.PROJECT_NAME,
            "avg_completion_time": avg_completion_time or "N/A",
            "incomplete_patient_count": incomplete_patient_count,
